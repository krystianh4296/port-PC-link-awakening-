...
impl Debugger {
        ...
        for w in self.watches.values_mut() {
            ...
            if w.kind == WatchType::Change && value != w.last_value {
                println!(
                    ...
                    w.address, w.last_value, value, self.frame
                );
                w.last_value = value;
            }
        }
        ...
        for offset in (0..length).step_by(16) {
            ...
            for i in 0..16 {
                ...
                line.push_str(&format!(
                    "{:02X} ",
                    bus.read(address.wrapping_add((offset + i) as u16))
                ));
            }
            println!("{}", line);
        }
    ...
    pub fn trace_instruction(&mut self, i: &DisassembledInstruction, cpu: &Cpu) {
        println!(
            "PC={:04X} OP={:02X}",
            i.address,
...
fn opcode_mnemonic(op: u8, b: &[u8], pc: u16) -> String {
    let n = || b.get(1).copied().unwrap_or(0);
    let nn = || {
        u16::from_le_bytes([
...
fn jr_text(pc: u16, o: u8) -> String {
    format!(
        "JR ${:04X}",
        pc.wrapping_add(2).wrapping_add((o as i8) as i16 as u16)
    )
}
    ...
    pub fn execute_console_command(
        ...
        match command {
            ...
            ConsoleCommand::Trace(start, end) => {
                ...
                println!("Trace enabled: {:04X} -> {:04X}", start, end);
            }

            ConsoleCommand::TraceSingle(address) => {
                self.set_trace_single(address);

                println!("Trace enabled: PC={:04X}", address);
            }
            ...
            ConsoleCommand::Break => {
                self.break_now(format!("Console break at PC={:04X}", cpu.pc));
