...
impl Ppu {
    ...
    fn update_stat_interrupt(&mut self) {
        ...
        let irq = (self.mode == 0 && self.stat & 0x08 != 0)
    ...
    pub fn background_tile_index(vram: &[u8; 0x2000], bg_x: u8, bg_y: u8, map_base: u16) -> u8 {
        let offset = (map_base - 0x8000) as usize + (bg_y as usize / 8) * 32 + (bg_x as usize / 8);
        vram[offset]
    }
    ...
    pub fn background_tile_data(vram: &[u8; 0x2000], tile_index: u8, base: u16) -> [u8; 16] {
        ...
        let mut tile = [0; 16]; tile.copy_from_slice(&vram[offset..offset + 16]); tile
    }

    pub fn decode_tile_row(tile: &[u8; 16], row: usize) -> [u8; 8] {
        let lo = tile[row * 2]; let hi = tile[row * 2 + 1];
        std::array::from_fn(|x| ((hi >> (7 - x)) & 1) << 1 | ((lo >> (7 - x)) & 1))
    }

    pub fn background_tile_attributes(vram: &[u8; 0x2000], bg_x: u8, bg_y: u8, map_base: u16) -> u8 {
        Self::background_tile_index(vram, bg_x, bg_y, map_base)
    }

    pub fn background_tile_attribute_info(a: u8) -> (u8, bool, bool, bool, bool) {
        (a & 7, a & 8 != 0, a & 0x20 != 0, a & 0x40 != 0, a & 0x80 != 0)
    }
