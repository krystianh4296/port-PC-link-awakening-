...
fn base_80_bf_alu_operations_produce_correct_results() {
    let cases = [
        ...
        (0x84, 0x10, 0x22, 0x32), // ADD H (register value is overwritten below)
        (0x85, 0x10, 0x22, 0x32), // ADD L
        (0x87, 0x10, 0x22, 0x32), // ADD A
        (0x88, 0x10, 0x22, 0x32), // ADC B, C=0
        (0x90, 0x32, 0x10, 0x22), // SUB B
        (0x98, 0x32, 0x10, 0x22), // SBC B, C=0
...
fn base_80_bf_add_adc_flags_are_correct() {
    let (cpu, _, _) = run(0x80, 0x0F, 0x01, 0x00); // 0F + 01 = 10
    assert_eq!(cpu.a, 0x10);
    assert_eq!(cpu.f, 0x20); // H

    let (cpu, _, _) = run(0x80, 0xFF, 0x01, 0x00); // FF + 01 = 00
    assert_eq!(cpu.a, 0x00);
    assert_eq!(cpu.f, 0xB0); // Z,H,C

    let (cpu, _, _) = run(0x88, 0x0F, 0x00, 0x10); // 0F + 0 + C = 10
    assert_eq!(cpu.a, 0x10);
    assert_eq!(cpu.f, 0x20); // H

    let (cpu, _, _) = run(0x88, 0xFF, 0x00, 0x10); // FF + 0 + C = 00
    assert_eq!(cpu.a, 0x00);
    assert_eq!(cpu.f, 0xB0); // Z,H,C
}
...
fn base_80_bf_sub_sbc_cp_flags_are_correct() {
    let (cpu, _, _) = run(0x90, 0x10, 0x01, 0x00); // 10 - 01 = 0F
    assert_eq!(cpu.a, 0x0F);
    assert_eq!(cpu.f, 0x60); // N,H

    let (cpu, _, _) = run(0x90, 0x00, 0x01, 0x00); // 00 - 01 = FF
    assert_eq!(cpu.a, 0xFF);
    assert_eq!(cpu.f, 0x70); // N,H,C

    let (cpu, _, _) = run(0x98, 0x10, 0x00, 0x10); // 10 - 0 - C = 0F
    assert_eq!(cpu.a, 0x0F);
    assert_eq!(cpu.f, 0x60); // N,H

    let (cpu, _, _) = run(0x98, 0x00, 0x00, 0x10); // 00 - 0 - C = FF
    assert_eq!(cpu.a, 0xFF);
    assert_eq!(cpu.f, 0x70); // N,H,C
...
#[test]
fn base_80_bf_hl_operand_is_used_and_flags_are_preserved_for_logic() {
    let mut memory = memory();
